   ...
   └─ PanelHost [Frame] — centered card host (`Size` on **Canvas** uses **scale**; live values ~**0.63 × 0.65** depend on mockup)
      ...
      └─ Panel [Frame] — navy card, gold stroke, UICorner
         ...
         └─ Body [Frame] — **`BackgroundTransparency = 1`**, **`ZIndex` 1**; holds the vertical column only
            ...
            │  │     ├─ **Canonical row `Frame` names:** `OilSellRowCrude` | `OilSellRowRefined` | `OilSellRowPremium` — same row chrome (`OilSellRow*` horizontal layout). Renamed rows still resolve via the **`SellOilTier`** attribute (`Crude` / `Refined` / `Premium`) or the row's `TypeName` text. **`SellOilPanel.luau`**: warehouse **stored** counts; Crude row sells the `Crude` tier only (no blending); prices from **`MarketPriceUpdate`**.
            ...
            │        *Runtime:* set **`RightText`** on subtotal rows — **canonical `Frame` names:** **`SubtotalCrude`**, **`SubtotalRefined`**, **`SubtotalPremium`** (one per tier). **`TotalSaleRow.RightText`** total. Remote **`RequestSellWarehouseBarrels`**.  
            └─ Footer [Frame]
