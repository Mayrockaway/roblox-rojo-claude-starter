...
|--------------|---------------|-------|
| `oil_sold`, `oilman_sold`, `instant_sold`, `dispatch_fee`, `equipment_purchased`, `chunk_unlocked`, `warehouse_upgraded`, `oil_tank_upgraded`, `ship_speed_upgraded`, `ship_capacity_upgraded` | `resource` | Soft currency. `flow_type = "Source" \| "Sink"`, `currency = "cash" \| "oil_<tier>"`, `amount = <int>`, `item_type = "<reason>"`, `item_id = "<sku>"`. |
...
| `ftue_step_abandoned` | `progression` (`Fail`) | `FtueService` `Players.PlayerRemoving` handler when player leaves mid-step (any step except `Done`) | `stepId = currentStep`, `funnelStep = ordinal`, `secondsInStep` |
...
| `player_died` | `design` (`player:died:<cause>`) | **Both:** `KillZoneService` death handler (`cause = "killzone"`) **and** `Humanoid.Died` (`cause = "unknown"` — falls, lava, future hazards). Per-character `Telemetry_DeathEmitted` flag prevents double-fire when the kill-zone path also kills the humanoid. | `cause` (`"killzone"`, `"airstrike"`, `"nuke"`, `"unknown"`), `position` (`{x,y,z}`), `streakBeforeDeath?` |
...
- **`KillZoneService`** death handler: `player_died` with `cause = "killzone"`. Also subscribe to `Humanoid.Died` (via `Player.CharacterAdded` → `character:WaitForChild("Humanoid").Died`) to cover non-killzone deaths (falls, lava, future hazards) with `cause = "unknown"` — both sites guarded by a per-character "already emitted" flag so a kill-zone hit doesn't double-fire (kill zones already kill the humanoid).
...
6. **PR 6 — Wire the remainder.** Everything else in §6/§7 in a single follow-up PR. Includes:
   - **Catalog cleanup:** drop `cash_granted_dev` (no caller exists today; revisit when admin/dev-grant tooling is built — the cleanest reintroduction is a dedicated `OilStateService:GrantDevCash(userId, amount, reason)` helper that is the only path which fires the event, so `skipMultiplier` refunds and Robux currency packs aren't falsely tagged as dev grants).
   - **Legacy `TelemetryService` migration:** move the surviving callers (`PersistenceService` → `profile_load_failed` / `profile_save_failed`, `StraitStateService` → `strait_force_lock`) onto `Shared.Telemetry`, then delete `src/server/Services/TelemetryService.luau` and remove its `Init`/`Start` wiring from `src/server/init.server.luau`.
   - **`ftue_step_abandoned`:** new event fired from `FtueService`'s `Players.PlayerRemoving` handler whenever the leaving player's current step is anything other than `Done` — gives us the FTUE drop-off curve (which step do players quit on?).
   - **`player_died` from both sources:** `KillZoneService` (cause `"killzone"`) and `Humanoid.Died` (cause `"unknown"` — falls/lava/future hazards), guarded by a per-character flag so the kill-zone path doesn't double-fire.
...
| 2026-04-22 | **PR 6 implemented (gameplay-systems-engineer).** Legacy `_telemetryService:Track` callers in `PersistenceService` / `StraitStateService` migrated to `Shared.Telemetry`; placeholder `src/server/Services/TelemetryService.luau` deleted and removed from `init.server.luau`; orphan `_telemetryService` parameter dropped from `PlayerStateService` / `UpgradeService` / `AdminService` (per-event placeholders dropped — see PR description). Wired §6.1–§6.7 events not already shipped in PRs 1–5: `first_join`, `ftue_started`, `ftue_step_abandoned`, `ftue_skipped`, `shipment_voyage_started`, `strait_waypoint_evaluated`, `strait_event_fired`, `shipment_total_loss`, `shipment_pirate_hit`, `shipment_pirate_hit_payout`, `security_personnel_consumed`, `security_personnel_deflected`, `player_died` (dedup via `Humanoid:SetAttribute("TelemetryDeathEmitted", true)` shared between `KillZoneService` + `Humanoid.Died` listener), `oil_pumped`, `oil_pump_collected`, `oilman_sold`, `instant_sold`, `equipment_purchased`, `equipment_purchase_failed`, `chunk_unlocked`, `warehouse_upgraded`, `oil_tank_upgraded`, `ship_speed_upgraded`, `ship_capacity_upgraded`, `ship_skin_equipped`, `equipment_shop_rotated`, `strait_state_changed` (with `cause` + `_lastEmittedState` dedup), `nuke_fired`, `airstrike_fired`, `pirate_hire_created`, `pirate_hire_refunded`, `pirate_hire_executed`, `monetization_prompted`, `monetization_prompt_failed`, `monetization_purchased_replay`, `monetization_grant_failed`, `gamepass_purchased`, `gamepass_prompt_cancelled`, `gamepass_owned_resolved`, `oil_state_load_failed`, `auto_ship_engaged`, `auto_ship_dispatched`, `auto_ship_skipped`, `dock_idle_to_active`. **Skipped:** `client_ready` (no existing client→server init handshake; left as a TODO for the next slice that wires one). |
